...
public:
	...
	int GetNumWalls()		  const		{ return numofwalls; }
	vector<int> CreateWalls();
	GraphNode* GetNodeFromPOS(Vector3 pos);

	//Used as a hack for the MazeRenderer to generate the walls more effeciently
	GraphNode* GetAllNodesArr() { return allNodes; }

	vector<float> GetPOSFromNodes(list<const GraphNode*> path);
	vector<Vector3> GetV3POSFromNodes(list<const GraphNode*> path);
