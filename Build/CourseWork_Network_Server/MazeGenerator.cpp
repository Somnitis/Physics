...
vector<int> MazeGenerator::CreateWalls() {
	
	for (int i = 0; i < numofwalls; i++) {
		Walls.push_back(allEdges[i]._iswall);
	}
	return Walls;
}

GraphNode* MazeGenerator::GetNodeFromPOS(Vector3 pos) {
	for (int i = 0;i < size*size;i++) {
		if (allNodes[i]._pos == pos) {
			return &allNodes[i];
		}
	}
	return nullptr;
...
vector<float> MazeGenerator::GetPOSFromNodes(list<const GraphNode*> path) {
	std::vector<const GraphNode*> v{ std::make_move_iterator(std::begin(path)),
...
vector<Vector3> MazeGenerator::GetV3POSFromNodes(list<const GraphNode*> path) {
	std::vector<const GraphNode*> v{ std::make_move_iterator(std::begin(path)),
		std::make_move_iterator(std::end(path)) };
	//Vector3 pos;
	vector<Vector3> NodePos;
	for (int i = 0; i < v.size(); i++)
	{
	//	pos = v[i]->_pos;
		NodePos.push_back(v[i]->_pos);
	}
	return NodePos;
}