		...
		for (size_t i = 0; i < physicsNodes.size() - 1; ++i)
		{
			for (size_t j = i + 1; j < physicsNodes.size(); ++j)
			{
					...
					&& pnodeB->GetCollisionShape() != NULL)
				{
					...
					if ((pnodeA->GetSphereRadius() + pnodeB->GetSphereRadius())*(pnodeA->GetSphereRadius() + pnodeB->GetSphereRadius()) >= Vector3::Dot(pnodeB->GetPosition() - pnodeA->GetPosition(), pnodeB->GetPosition() - pnodeA->GetPosition())) {
						CollisionPair cp;
						cp.pObjectA = pnodeA;
						cp.pObjectB = pnodeB;
	...
	if (broadphaseColPairs.size() > 0)
	{
		...
		for (size_t i = 0; i < broadphaseColPairs.size(); ++i)
		{
			...
			if (colDetect.AreColliding(&colData))
			{
				...
				{
					NCLDebug::DrawPointNDT(colData._pointOnPlane, 3.1f, Vector4(0.5f, 0.5f, 1.0f, 1.0f));
